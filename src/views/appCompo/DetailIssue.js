...
function DetailIssue({User}) {
    ...
    return (
        <Grid container style={{flexGrow: 1}} spacing={1}>
            ...
            <Grid item xs={12}
                  sx={{display: 'flex', marginBottom: 2, justifyContent: 'space-between', alignItems: 'center'}}>
                <Typography variant='h5'> Tags : {User?.fields?.labels.map((label, index) =>
                    <Chip key={index} color='error' size='small' variant='outlined'
                          label={label}></Chip>)}   </Typography>
                <Typography variant='h6'> <KeyboardDoubleArrowUpIcon fontSize="small"
                                                                     color='error'/> {User?.fields?.priority.name}
                </Typography>
            </Grid>
            ...
            <Grid item xs={12} sx={{display: 'flex', justifyContent: 'flex-start', alignItems: 'center'}}>
                <MainCard style={{display: 'flex', width: '100%', height: '200px', overflow: 'auto'}}
                          sx={{border: 1, borderColor: '#d2d6de'}}>
                    <Typography variant='body2'>{User?.fields?.description} </Typography>
                </MainCard>
            </Grid>
            <Grid item xs={12} sx={{display: 'flex', justifyContent: 'flex-start', alignItems: 'center'}}>
                <Typography onClick={handleClickFile} style={{cursor: 'pointer'}} variant='body1'> File
                    : {User?.fields?.attachment[0]?.filename} <AttachFileIcon color={'primary'}
                                                                              fontSize='small'/></Typography>

            </Grid>
            <Grid item xs={12} marginTop={2} sx={{display: 'flex', justifyContent: 'flex-end', alignItems: 'center'}}>
                <Stack direction="row" spacing={2}>
                    <Button disabled={!User} onClick={handleDelete} size='small' color='inherit' variant="outlined"
                            startIcon={<DeleteIcon/>}>
                        Delete
                    </Button>
                    <Button disabled={!User} onClick={handleComplete} size='small' color='secondary' variant="contained"
                            endIcon={<SendIcon/>}>
                        Complet
                    </Button>
                </Stack>
